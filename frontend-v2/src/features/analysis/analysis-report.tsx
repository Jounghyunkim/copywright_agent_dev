...
/** Section help 키 → i18n resource key. 실제 텍스트는 analysis:sectionHelp 경유. */
...
export function AnalysisReport({
  ...
  const help = (title: string) =>
    onHelpRequest
      ? () => onHelpRequest(title, t(`analysis:sectionHelp.${title}`, { defaultValue: '' }))
      : undefined
    ...
    <Card style={{ padding: '1.2rem' }}>
      ...
      >
        ...
        {!isApproved && onApprove && (
          ...
          >
            <Button onClick={onApprove}>{t('analysis:button.approve')}</Button>
          </div>
        )}
      </div>
    </Card>
